...
#-------------------------------------------------------------------
    ...
    if page == "매매":
        ...
        st.sidebar.title('연립/다세대_매매')
         ...
         # 데이터 불러오기, 데이터 형 변환.
        df = pd.read_csv('C:\\Users\\innov\\workspace\\realestate_detective1_rawdata\\수도권_연립다세대_매매\\연립다세대_매매_seoul+ggd.csv', encoding="CP949")
        df['계약년월'] = pd.to_datetime(df['계약년월'], format='%Y%m')  # 날짜 형식으로 변환
        # df['계약년월'] = pd.to_numeric(df['계약년월'], errors='coerce')
        df['전용면적(㎡)'] = pd.to_numeric(df['전용면적(㎡)'], errors='coerce')  # 전용면적을 숫자로 변환
        df['거래금액(만원)'] = pd.to_numeric(df['거래금액(만원)'], errors='coerce') # 거래금액을 숫자로 변환 / 엑셀에서 통화로 표시된 유형을 일반으로 바꿔줘야함 !!
        ...
        plt.xlabel('년도')
        plt.ylabel('평균 거래금액(만원)')
        plt.title(f'{selected_district} 연립/다세대 시세 - 시간별')
        plt.xticks(rotation=45)
        st.pyplot(plt)
        ...
        # 데이터 불러오기, 데이터 형 변환.
        df = pd.read_csv('C:\\Users\\innov\\workspace\\realestate_detective1_rawdata\\수도권_연립다세대_전월세\\연립다세대_전월세_seoul+ggd.csv', encoding="CP949")
        df['계약년월'] = pd.to_datetime(df['계약년월'], format='%Y%m')  # 날짜 형식으로 변환
        # df['계약년월'] = pd.to_numeric(df['계약년월'], errors='coerce')
        df['전용면적(㎡)'] = pd.to_numeric(df['전용면적(㎡)'], errors='coerce')  # 전용면적을 숫자로 변환
        ...
        if lease_type == '전세':
            ...
            plt.xlabel('년도')
            plt.ylabel('평균 거래금액(만원)')
            plt.title(f'{selected_district} 연립/다세대 전세 - 시간별')
            plt.xticks(rotation=45)
            st.pyplot(plt)
            ...
            plt.xlabel('년도')
            plt.ylabel('평균 거래금액(만원)')
            plt.title(f'{selected_district} 연립/다세대 월세 보증금 - 시간별')
            plt.xticks(rotation=45)
            st.pyplot(plt)
            ...
            plt.xlabel('년도')
            plt.ylabel('평균 거래금액(만원)')
            plt.title(f'{selected_district} 연립/다세대 월세 - 시간별')
            plt.xticks(rotation=45)
            st.pyplot(plt)
...
#-------------------------------------------------------------------
    ...
    with st.sidebar:
        page = st.radio('거래유형', ['매매', '전월세'])
    if page == "매매":
        st.header(":house: 단독/다가구")
        st.title("서울-경기 매매동향")
        st.sidebar.title('단독/다가구_매매')

         # 데이터 불러오기, 데이터 형 변환.
        df = pd.read_csv('C:\\Users\\innov\\workspace\\realestate_detective1_rawdata\\수도권_단독다가구_매매\\수도권_단독다가구_매매_seoul+ggd.csv', encoding="CP949")
        df['계약년월'] = pd.to_datetime(df['계약년월'], format='%Y%m')  # 날짜 형식으로 변환
        # df['계약년월'] = pd.to_numeric(df['계약년월'], errors='coerce')
        df['연면적(㎡)'] = pd.to_numeric(df['연면적(㎡)'], errors='coerce')  # 연면적을 숫자로 변환
        df['거래금액(만원)'] = pd.to_numeric(df['거래금액(만원)'], errors='coerce') # 거래금액을 숫자로 변환 / 엑셀에서 통화로 표시된 유형을 일반으로 바꿔줘야함 !!
        ...
        # 필터 옵션 설정2_ 아파트 연면적별(multiselect로 수정)

        selected_areas = st.sidebar.multiselect('연면적(㎡) 선택', ['40이하', '40-60', '60-74', '74-85', '85~101', '101-124', '124-140', '140이상'])

        area_conditions = []            #사용자가 선택한 각 연면적 범위 조건이 담김.

        for selected_area in selected_areas:
            if selected_area == '40이하':
                area_conditions.append((filtered_df['연면적(㎡)'] >= 0) & (filtered_df['연면적(㎡)'] <= 40))
            elif selected_area == '40-60':
                area_conditions.append((filtered_df['연면적(㎡)'] >= 40) & (filtered_df['연면적(㎡)'] <= 60))
            elif selected_area == '60-74':
                area_conditions.append((filtered_df['연면적(㎡)'] >= 60) & (filtered_df['연면적(㎡)'] <= 74))
            elif selected_area == '74-85':
                area_conditions.append((filtered_df['연면적(㎡)'] >= 74) & (filtered_df['연면적(㎡)'] <= 85))
            elif selected_area == '85~101':
                area_conditions.append((filtered_df['연면적(㎡)'] >= 85) & (filtered_df['연면적(㎡)'] <= 101))
            elif selected_area == '101-124':
                area_conditions.append((filtered_df['연면적(㎡)'] >= 101) & (filtered_df['연면적(㎡)'] <= 124))
            elif selected_area == '124-140':
                area_conditions.append((filtered_df['연면적(㎡)'] >= 125) & (filtered_df['연면적(㎡)'] <= 140))
            elif selected_area == '140이상':
                area_conditions.append((filtered_df['연면적(㎡)'] >= 140))
        ...
        plt.xlabel('년도')
        plt.ylabel('평균 거래금액(만원)')
        plt.title(f'{selected_district} 단독다가구 시세 - 시간별')
        plt.xticks(rotation=45)
        st.pyplot(plt)
        ...
        # 데이터 불러오기, 데이터 형 변환.
        df = pd.read_csv('C:\\Users\\innov\\workspace\\realestate_detective1_rawdata\\수도권_단독다가구_전월세\\단독다가구_전월세_seoul+ggd.csv', encoding="CP949")
        df['계약년월'] = pd.to_datetime(df['계약년월'], format='%Y%m')  # 날짜 형식으로 변환
        # df['계약년월'] = pd.to_numeric(df['계약년월'], errors='coerce')
        df['계약면적(㎡)'] = pd.to_numeric(df['계약면적(㎡)'], errors='coerce')  # 계약면적을 숫자로 변환
        ...
        # 필터 옵션 설정3_ 아파트 계약면적별(multiselect로 수정)

        selected_areas = st.sidebar.multiselect('계약면적(㎡) 선택', ['40이하', '40-60', '60-74', '74-85', '85~101', '101-124', '124-140', '140이상'])

        area_conditions = []            #사용자가 선택한 각 계약면적 범위 조건이 담김.

        for selected_area in selected_areas:
            if selected_area == '40이하':
                area_conditions.append((filtered_df['계약면적(㎡)'] >= 0) & (filtered_df['계약면적(㎡)'] <= 40))
            elif selected_area == '40-60':
                area_conditions.append((filtered_df['계약면적(㎡)'] >= 40) & (filtered_df['계약면적(㎡)'] <= 60))
            elif selected_area == '60-74':
                area_conditions.append((filtered_df['계약면적(㎡)'] >= 60) & (filtered_df['계약면적(㎡)'] <= 74))
            elif selected_area == '74-85':
                area_conditions.append((filtered_df['계약면적(㎡)'] >= 74) & (filtered_df['계약면적(㎡)'] <= 85))
            elif selected_area == '85~101':
                area_conditions.append((filtered_df['계약면적(㎡)'] >= 85) & (filtered_df['계약면적(㎡)'] <= 101))
            elif selected_area == '101-124':
                area_conditions.append((filtered_df['계약면적(㎡)'] >= 101) & (filtered_df['계약면적(㎡)'] <= 124))
            elif selected_area == '124-140':
                area_conditions.append((filtered_df['계약면적(㎡)'] >= 125) & (filtered_df['계약면적(㎡)'] <= 140))
            elif selected_area == '140이상':
                area_conditions.append((filtered_df['계약면적(㎡)'] >= 140))
        ...
        if lease_type == '전세':
            ...
            plt.xlabel('년도')
            plt.ylabel('평균 거래금액(만원)')
            plt.title(f'{selected_district} 단독다가구 전세 - 시간별')
            plt.xticks(rotation=45)
            st.pyplot(plt)
            ...
            plt.xlabel('년도')
            plt.ylabel('평균 거래금액(만원)')
            plt.title(f'{selected_district} 단독다가구 월세 보증금 - 시간별')
            plt.xticks(rotation=45)
            st.pyplot(plt)
            ...
            plt.xlabel('년도')
            plt.ylabel('평균 거래금액(만원)')
            plt.title(f'{selected_district} 단독다가구 월세 - 시간별')
            plt.xticks(rotation=45)
            st.pyplot(plt)
